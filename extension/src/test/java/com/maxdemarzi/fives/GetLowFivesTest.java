...
public class GetLowFivesTest {
    ...
    public void shouldGetHighFives() {
        ...
        HTTP.Response response = HTTP.GET(neo4j.httpURI().resolve("/v1/users/maxdemarzi/low_fives").toString());
        ArrayList<HashMap> actual  = response.content();
        Assert.assertTrue(actual.size() == 2);
        Assert.assertEquals(actual.get(0).get("low_fives"), 1);
    ...
    private static final String FIXTURE =
            "CREATE (max:User {username:'maxdemarzi', " +
                    ...
                    "CREATE (jexp)-[:LOW_FIVED {time: datetime('%s')}]->(post1)" +
                    "CREATE (laeg)-[:LOW_FIVED {time: datetime('%s')}]->(post2)" +
